...
def _create_session(
    ...
    ``VramMonitor`` requires that population, so a CUDA trial without it is refused here with an
    actionable error rather than a ``TypeError`` raised deeper in the run (D-046).
    """
