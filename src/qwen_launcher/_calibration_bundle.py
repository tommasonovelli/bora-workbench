...
class BundleWorkspace:
    ...
    def finalize(
        ...
        logs = _copy_logs(self.staging_path, target, trials)
        context = DocumentContext(self.report_id, self.created_at, target, settings, trials, logs)
        report_name = f"{self.report_id}.json"
        _write_json(self.staging_path / report_name, build_report(context))
        _write_json(self.staging_path / "benchmark-results.json", build_benchmarks(context))
        _write_json(self.staging_path / "profile-proposal.json", build_profile_proposal(context))
...
def _copy_logs(staging: Path, target: CalibrationTarget, trials: TrialResult) -> LogEvidence:
    """Copy redacted logs and return report-relative SHA-256 evidence by mode/candidate."""
    evidence: LogEvidence = {}
    private_values = _private_values(target, staging)
