"""Offline tests for the calibration/v6-lite bounded feasibility search."""
...
from qwen_launcher._calibration_v6_types import V6SearchError
...
def test_persistent_retryable_and_protocol_invalid_stop_the_mode() -> None:
    ...
    with pytest.raises(V6SearchError, match="retryable"):
        search_step(lambda value: ClassifiedOutcome(TrialOutcome.RETRYABLE), budget=24)
    with pytest.raises(V6SearchError, match="protocol-invalid"):
