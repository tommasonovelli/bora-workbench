...
def test_redirected_progress_prints_only_completed_trials() -> None:
    ...
    started = ProgressEvent("coding", "screening", 0, 12, None, True)
    completed = ProgressEvent("coding", "screening", 1, 12, 110.0)

    with CalibrationProgress(console) as progress:
        progress(started)
        progress(completed)

    assert stream.getvalue() == "coding screening: 1/≤12; cap projection ≈ 1m 50s\n"
...
def test_terminal_progress_retains_a_phase_summary() -> None:
    ...
    with CalibrationProgress(console, get_time) as progress:
        progress(ProgressEvent("coding", "screening", 0, 12, None, True))
        now[0] = 120.0
        progress(ProgressEvent("coding", "screening", 1, 12, 110.0))

    rendered = stream.getvalue()
    assert "coding screening complete: 1 trial(s) in 2m 00s." in rendered
    assert "coding screening: 1/≤12" not in rendered
