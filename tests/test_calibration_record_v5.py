"""Offline roundtrip tests for the lean calibration-record/v5 document."""
...
from qwen_launcher._calibration_v6_types import EnvelopeResult, GateResult
...
def test_v5_record_roundtrips_and_reconstructs_selection(tmp_path) -> None:
    """Build, atomically write, and reload a v5 record with its selection inputs intact."""
    context = V6RecordContext(_target(), "run-abc123", "balanced", "test-driver")
    document = build_record_v5(context, _mode_result())
...
def test_v5_record_rejects_tampered_reserves(tmp_path) -> None:
    """Reject a v5 record whose reserves no longer match the pinned v6 constants."""
    context = V6RecordContext(_target(), "run-abc123", "balanced", "test-driver")
    document = build_record_v5(context, _mode_result())
...
def test_v5_reuse_reads_active_envelope_and_headroom() -> None:
    """Select the active envelope and apply the record's own reserves at reuse time."""
    from qwen_launcher._calibration_reuse import _active_envelope, _headroom_v5

    document = build_record_v5(
        V6RecordContext(_target(), "run-abc123", "max_context", "test-driver"), _mode_result()
    )
    assert _active_envelope(document)["ctx"] == 131072
    assert _headroom_v5(document, vram_free_gib=10.0, ram_gib=10.0) == []
    assert _headroom_v5(document, vram_free_gib=10.0, ram_gib=3.0) != []
    assert _headroom_v5(document, vram_free_gib=1.0, ram_gib=10.0) != []
