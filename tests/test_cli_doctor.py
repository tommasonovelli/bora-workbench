...
def test_doctor_distinguishes_record_lifecycle_states() -> None:
    """Present all seven record states with their canonical shared labels."""
    evaluations = {
        "active balanced cell valid": RecordEvaluation(
            "valid", 8192, None, (), preference="balanced"
        ),
        "active record is absent": RecordEvaluation("missing", None, None, ()),
        "candidate awaits activation": RecordEvaluation(
            "candidate", None, None, ("pending",), "valid"
        ),
        "superseded": RecordEvaluation("superseded", None, None, ("rerun calibrate",)),
        "incompatible": RecordEvaluation("incompatible", 8192, None, ("engine changed",)),
        "invalid": RecordEvaluation("invalid", None, None, ("broken JSON",)),
        "insufficient headroom": RecordEvaluation(
            "insufficient-headroom", 8192, None, ("free VRAM low",)
        ),
    }
...
@pytest.mark.parametrize(
    ("candidate_status", "diagnostics", "expected"),
    (
        ("missing", (), ""),
        ("valid", ("pending",), "Pending candidate is valid"),
        ("invalid", ("broken candidate",), "Pending candidate is invalid: broken candidate"),
        (
            "superseded",
            ("old candidate",),
            "Pending candidate is superseded: old candidate",
        ),
    ),
)
def test_doctor_keeps_candidate_as_a_secondary_fact(
    candidate_status, diagnostics, expected
) -> None:
    """Render every candidate status beside, never instead of, the active state."""
    ...
    assert "active balanced cell" in line
    if expected:
        assert expected in line
    else:
        assert "Pending candidate" not in line
