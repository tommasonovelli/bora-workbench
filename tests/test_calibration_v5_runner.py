"""Offline calibration/v5 orchestration tests with fake processes and paired evidence."""
...
import qwen_launcher._calibration_v5_confirm as confirm_module
import qwen_launcher._calibration_v5_mode as mode_module
import qwen_launcher._calibration_v5_runner as runner_module
import qwen_launcher._calibration_v5_screening as screening_module
...
from qwen_launcher._calibration_v5_process import TrialFailure, TrialMeasurement
from qwen_launcher._calibration_v5_runner import run_calibration_v5
from qwen_launcher._calibration_v5_types import TrialEvidence, V5RunOptions
...
def test_cuda_search_runs_abba_and_activates_a_valid_record(tmp_path, monkeypatch) -> None:
    ...
    outcome = run_calibration_v5(cuda_target(), destination_root=tmp_path)
...
def test_no_activate_retains_candidate_and_prior_active(tmp_path, monkeypatch) -> None:
    ...
    outcome = run_calibration_v5(
        cuda_target(), V5RunOptions(is_activate=False), destination_root=tmp_path
    )
...
def test_cpu_confirms_two_benchmarked_baseline_rounds(tmp_path, monkeypatch) -> None:
    ...
    run_calibration_v5(cpu_target(), destination_root=tmp_path)
...
def test_discarded_finalists_and_environment_errors_fail_honestly(tmp_path, monkeypatch) -> None:
    ...
    with pytest.raises(CalibrationError, match="no finalist passed"):
        run_calibration_v5(cuda_target(), destination_root=tmp_path / "discarded")
    ...
    with pytest.raises(CalibrationError, match="run invalidated"):
        run_calibration_v5(cuda_target(), destination_root=tmp_path / "invalid")
