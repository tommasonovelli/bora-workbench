...
| D-098 | On 31 July 2026 the maintainer reports that a record refuses the machine that produced it, and directs the correction as `0.5.3`. Each memory reserve was charged twice against the same bytes. A trial refuses every candidate that drives free memory below its reserve (section 5.6), so a recorded need already leaves that margin behind; reuse then asked for the need **plus** the reserve a second time, and compared the sum against a free-memory figure that already has this machine's current foreign usage subtracted. The second charge therefore set aside a margin for a hypothetical workload on top of the real one already counted, which made the reserve unusable by construction: the moment other applications took the memory the reserve exists to lend them, the record that reserve protects was voided. On the maintainer's `vstudio` record the measured VRAM need is `6.628` GiB and the trial itself left `0.765` GiB free, but reuse demanded `7.128` GiB against `7.010` free, so `0.38` GiB of ordinary desktop drift cost an eightfold context window and a silent fall to the `ctx=8192` baseline. Reuse now requires the measured RAM need and, on CUDA, the measured VRAM need, and nothing beyond them; a record that clears its need while leaving less free than its own reserve is served with a warning naming the remainder, and that warning never refuses the cell. The RAM branch changes with it because it is the same defect, and because `enforce_memory_gate` already blocks the pinned model below 22 GiB available — above the need any record measures — which closes a window in which a launch passed that hard gate and then degraded in silence. The trial half of D-042 stands; its reuse half is corrected here. The trial monitors, the pinned `0.5`/`2.0`/`0.125` reserves, their exact-equality verification, `calibration-record/v6`, its schema, the fallback, and `command_contract_sha256` are all unchanged, so every existing record stays valid and no recalibration is required: this change is what makes those records usable again. After the complete frozen suite, lint, format, packaged-content validation, build, isolated wheel verification, and diff inspection, this authorizes the correction commit and the `0.5.3` finalization commit on `main`. The whole CI job was additionally reproduced locally on the Windows host, step for step and in order, and the correction was exercised against the maintainer's own records: `doctor` reports all three valid where every one had been refused, and `bora vstudio` reaches `model loaded` at `n_ctx_slot = 65536` with `0.15` GiB of VRAM left free. On the same day, after reading that result, the maintainer authorizes the push to `main` and the tag `v0.5.3`; the GitHub Release is authorized only after that tag's Ubuntu/Windows workflow is green and only from its exact bundle, which is the sole thing that may be attached to it. The Ubuntu leg was never run locally and is not claimed. GitHub Releases remains the only distribution channel; no registry upload, candidate activation, or Gate claim is authorized. |
...
- available RAM ≥ the record's measured requirement;
- CUDA free VRAM ≥ the record's measured requirement.

Each reserve is charged once, where it is measured: section 5.6 refuses every trial candidate that
drives free memory below it, so a recorded requirement already leaves its reserve behind, and what
is free now already has this machine's current foreign usage subtracted. A record that clears its
requirement but leaves less than its own reserve is served with a note naming the remainder; the
note never refuses the cell (D-098).
